...
Ở đây các bạn sẽ phải sửa hàm **collide** () của tất cả các đối tượng **Bomber** , **Enemy** , **Wall** , **Brick** , **Bomb**. Các hàm nhận tham số là một đối tượng Entity bất kỳ này trả về true nếu xác định có va chạm với đối tượng đó và false trong trường hợp không va chạm. Đồng thời, hàm cũng xử lý các sự kiện cần thiết trong game:
...
Với phương thức **placeBomb** (), bạn chỉ đơn giản là thêm một đối tượng **Bomb** vào bản đồ ( **\_gameBoard** ).
...
Xử lý khi đi vào vị trí **Portal** , tức là kết thúc game, bạn cần chỉnh sửa lại hàm **collide** () trong lớp **Portal**. Lưu ý là khi tất cả các kẻ địch đều bị tiêu diệt thì mới có thể kết thúc trò chơi. Để kết thúc màn chơi cũ và chuyển sang màn mới, hãy sử dụng **\_gameBoard.levelUp** ().
